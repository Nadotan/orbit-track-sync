...
type CreateTaskMode = "task" | "subtask";
...
function TasksPage() {
  ...
  const [createTaskMode, setCreateTaskMode] = useState<CreateTaskMode>("task");
  ...
  async function handleCreate() {
    ...
    try {
      ...
      toast.success(createTaskMode === "subtask" ? "Subtask created" : "Task created");
      setCreateOpen(false);
      await load();
    } catch (error) {
      toast.error(
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6 pb-28 md:pb-10">
          ...
          <section className="space-y-3">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Open a project for its overview, subtasks and project updates.
              </p>
            </div>
      ...
      {canCreate && selectedProject?.canEditDetails && projectPageId && (
        <FloatingSubtaskButton onClick={() => openNewTask(selectedProject)} />
      )}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>
              {createTaskMode === "subtask" ? "New Subtask" : "New Task"}
            </DialogTitle>
            <DialogDescription>
              {createTaskMode === "subtask"
                ? "Project and team are preselected. This is still a regular task and can be moved later."
                : "Create a task. You can attach it to a project now or later."}
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {incompleteForConfirmation} subtask
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Subtasks use the exact same task cards and behavior as every other
              task.
            </p>
          </div>
        ...
        <TaskList
          ...
          emptyText="Add the first subtask when this project is ready to break down."
          onOpen={onOpenTask}
          actions={taskActions}
        />
      </section>
      ...
      <WorkUpdatesPanel
        ...
        description="Updates written directly on this project. Subtask activity stays with each subtask."
...
function FloatingSubtaskButton({ onClick }: { onClick: () => void }) {
  return (
    <div className="fixed bottom-24 right-4 z-40 md:bottom-6 md:right-6">
      <Button
        type="button"
